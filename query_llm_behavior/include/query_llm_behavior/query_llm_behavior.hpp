 ...
 * @brief Behavior Tree node that queries a multimodal LLM using model name,
 * prompt and optional images.
 *
 * Returns the LLM response as a string via an output port. A subclass may
 * override `parse_llm_answer()` to convert the raw string into a structured
 * object, which is then provided via a separate output port.
 */
...
/**
 * @brief Behavior Tree node that queries a vision-capable LLM using a textual
 * prompt and an optional sequence of base64-encoded images.
 *
...
public:
   ...
   * - Input:
   *   - `llm_model` (`std::string`, optional): Model identifier (e.g. "llava").
   * Defaults to `"llava"`.
   *   - `llm_prompt` (`std::string`, required): Prompt to be passed to the
   * model.
   *   - `image_list` (`std::shared_ptr<std::vector<sensor_msgs::msg::Image>>`):
   * Images with metadata.
   *
   * - Output:
   *   - `llm_answer` (`std::shared_ptr<std::string>`): Raw string response from
   * the LLM.
   *   - `llm_parsed_answer` (`std::shared_ptr<ParsedType>`): Optionally parsed
   * structured result.
   */
  inline static BT::PortsList providedPorts() {
    return {
        ...
        BT::InputPort<std::shared_ptr<std::vector<sensor_msgs::msg::Image>>>(
            "image_list"),
  ...
  /**
   * @brief Invoked once at the beginning. This method retrieves inputs (model
   * name, prompt, image list), loads the model, and runs inference on it.
   *
  ...
  inline BT::NodeStatus onStart() override {
    ...
    if (auto llm_model_exp = getInput<std::string>("llm_model");
        llm_model_exp) {
      llm_model = llm_model_exp.value();
    } else {
      RCLCPP_WARN(this->get_logger(),
                  "Input [llm_model] not specified. Using default: %s",
                  default_llm_model_);
    ...
    // Preload model with timeout using scoped future
    {
      ...
      if (loader.wait_for(preload_timeout_) == std::future_status::timeout) {
        throw BT::RuntimeError(
            "Loading LLM model timed out after " +
      ...
      try {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Unable to load model due to the following exception, "
                    "returning FAILURE: %s",
                    e.what());
    ...
    inference_future_ = std::async(
        std::launch::async, [this, llm_model, llm_prompt, image_list]() {
  ...
  /**
   * @brief If onStart() returned RUNNING, this method is constantly called
   * until it returns something other than RUNNING. Checks whether LLM inference
   * has successfully completed and sets the output ports accordingly.
   *
  ...
  inline BT::NodeStatus onRunning() override {
    ...
    if (inference_future_.valid() &&
        inference_future_.wait_for(std::chrono::milliseconds(0)) ==
      ...
      try {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Caught exception during inference, returning FAILURE: %s",
                    e.what());
        return BT::NodeStatus::FAILURE;
      }
...
protected:
  /**
   * @brief Optionally override this in subclasses to extract structured data
   * from the LLM output.
   *
   * If overridden and returns a non-null result, it will be published to the
   * `llm_parsed_answer` port.
   *
   * @param raw_answer The raw string returned from the LLM.
   * @return A shared pointer to the parsed result (or nullptr if unused).
   */
  inline virtual std::shared_ptr<ParsedType>
  parse_llm_answer([[maybe_unused]] const std::string &raw_answer) {
    return nullptr;
  }

private:
  /**
   * @brief Ensures that the specified LLM model is available locally by
   * checking, pulling, and loading as needed.
   *
   * This method first checks the list of locally available models to determine
   * whether the target model is already cached. If not found, it attempts to
   * pull it from the Ollama server. After pulling, it verifies the model can be
   * loaded.
   *
    ...
    try {
      ...
      if (found != models.end()) {
        RCLCPP_INFO(this->get_logger(),
                    "Model [%s] is already available locally.",
                    llm_model.c_str());
        return;
      }
    } catch (const std::exception &e) {
      throw std::runtime_error("Failed to list local models: " +
                               std::string(e.what()));
    }
    ...
    try {
      if (!ollama::pull_model(llm_model)) {
        throw std::runtime_error(
            "Failed to pull model: " + llm_model +
            ". The model may not exist or the name may be incorrect.");
      }
    } catch (const std::exception &e) {
      throw std::runtime_error("Exception occurred while pulling model [" +
                               llm_model + "]: " + std::string(e.what()));
    }

    // Load the model into memory after successful pull
    RCLCPP_INFO(this->get_logger(),
                "Model [%s] successfully pulled. Attempting to load...",
                llm_model.c_str());

    try {
      if (!ollama::load_model(llm_model)) {
        throw std::runtime_error("Model pulled but still failed to load: " +
                                 llm_model);
      }
    } catch (const std::exception &e) {
      throw std::runtime_error("Exception occurred while loading model [" +
                               llm_model + "]: " + std::string(e.what()));
    }

    RCLCPP_INFO(this->get_logger(), "Model [%s] successfully loaded.",
                llm_model.c_str());
  }
  ...
  get_llm_answer_(const std::string &llm_model, const std::string &llm_prompt,
    ...
    img_order
        << "Images have been stitched side‑by‑side in the following order:\n";
   ...
   *
   * @note
   *   - All images must have the same type; otherwise, mismatched ones are
   * skipped.
   *   - Images with smaller height are zero-padded (black) at the bottom to
   * match the tallest one.
   *   - Logs warnings or errors with the descriptive @c frame_id of each image
   * for traceability.
   */
  ...
  inline cv::Mat stitch_images_horizontally_(
    ...
    for (const auto &ros_img : ros_imgs) {
      ...
      try {
        ...
        if (img.empty()) {
          RCLCPP_WARN(this->get_logger(), "Image [%s] is empty; skipping.",
                      frame_id.c_str());
          continue;
        }
        ...
        if (img.type() != ref_type) {
          RCLCPP_WARN(this->get_logger(),
                      "Image [%s] has mismatched type. Expected type %d but "
                      "got %d. Skipping.",
        ...
        RCLCPP_ERROR(this->get_logger(),
                     "Image [%s] could not be converted: %s", frame_id.c_str(),
                     e.what());
      }
    }
    ...
    for (auto &img : mats) {
      if (img.rows < max_rows) {
        ...
        cv::copyMakeBorder(img, img, 0, pad, 0, 0, cv::BORDER_CONSTANT,
                           cv::Scalar(0, 0, 0));
      }
    }
  ...
  /**
   * @brief Converts a cv::Mat image to a base64-encoded string using the
   * specified image format.
   *
   * This function first encodes the input image into the desired format (e.g.,
   * JPEG or PNG), and then converts the resulting binary data into a
  ...
  /**
   * @brief Converts a ROS2 sensor_msgs::msg::Image message to a base64-encoded
   * JPEG string.
   *
   * This function uses cv_bridge to convert a ROS2 image message into an OpenCV
   * image, and then compresses and base64-encodes it using JPEG format.
   *
   ...
   */
  inline std::string
  convert_msg_to_base64_(const sensor_msgs::msg::Image &ros_image) {
    try {
      ...
      cv_ptr = cv_bridge::toCvCopy(
          msg, sensor_msgs::image_encodings::RGB8); // Assuming all incoming ROS
                                                    // msgs have this encoding
   ...
   *
   * This function can be used for debugging purposes to inspect intermediate
   * image data.
   *
   * @param img ROS sensor_msgs::msg::Image to be saved.
   * @param filename Target path to save the JPEG image (e.g.,
   * "/tmp/debug.jpg").
   * @throws std::runtime_error if the image cannot be decoded or saved.
   */
  inline void save_image_to_file(const sensor_msgs::msg::Image &img,
                                 const std::string &filename) {
    try {
      const auto msg = std::make_shared<sensor_msgs::msg::Image>(img);
      cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(
          msg,
          sensor_msgs::image_encodings::BGR8); // OpenCV expects this encoding

      if (!cv::imwrite(filename, cv_ptr->image)) {
        throw std::runtime_error("Failed to write image to file: " + filename);
      }
    } catch (const std::exception &e) {
      throw std::runtime_error("Image debug save failed: " +
                               std::string(e.what()));
    }
  }

  /**
   * @brief Utility function to save a base64-encoded image to a binary file for
   * debugging.
   *
   * Decodes the base64 string and writes the resulting binary image content to
   * the file.
   *
   * @param base64_img The base64-encoded image string.
   * @param filename The path to write the decoded image binary to (e.g.,
   * "/tmp/image.jpg").
   * @throws std::runtime_error if decoding or writing fails.
   */
  inline void save_image_to_file(const std::string &base64_img,
                                 const std::string &filename) {
    try {
      ...
      throw std::runtime_error("Failed to save base64 image to file: " +
                               std::string(e.what()));
    }
  }
  ...
  const std::chrono::steady_clock::duration preload_timeout_ =
